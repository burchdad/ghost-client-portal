...
export default async function AdminProposalsPage({
  ...
  const proposals = await getDb().proposal.findMany({
    ...
    include: {
      organization: true,
      acceptances: { take: 1, orderBy: { acceptedAt: "desc" } },
    },
  ...
  return (
    <section>
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-2 text-sm text-muted">
            Manage portal-specific client proposal records.
          </p>
        </div>
        <Link
      ...
      <form className="mt-6 grid gap-3 rounded-lg border border-line bg-panel p-4 md:grid-cols-[1fr_220px_auto]">
        <input
          name="q"
          placeholder="Search organization or title"
      ...
      <div className="mt-6 space-y-3">
        {proposals.map((proposal) => (
          <Link
            ...
            className="block rounded-lg border border-line bg-panel p-5 hover:border-accent"
          >
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-muted">
                  Viewed:{" "}
                  {proposal.viewedAt
                    ? proposal.viewedAt.toLocaleString()
                    : "not yet"}{" "}
                  · Accepted: {proposal.acceptances[0]?.signerName ?? "not yet"}
                </p>
              </div>
