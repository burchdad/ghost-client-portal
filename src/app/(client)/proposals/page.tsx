    ...
    <section>
      ...
      <div className="mt-6 space-y-3">
        {proposals.map((proposal) => (
          <div key={proposal.id} className="rounded-lg border border-line bg-panel p-5">
            <p className="text-sm text-accent">{proposal.proposalNumber}</p>
            <h2 className="mt-1 text-xl font-semibold">{proposal.title}</h2>
            <p className="mt-2 text-sm text-muted">{proposal.status.replaceAll("_", " ").toLowerCase()}</p>
          </div>
        ))}
      </div>
    </section>
