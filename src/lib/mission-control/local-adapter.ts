...
export class LocalMissionControlClient implements MissionControlClient {
  ...
  async createMissionControlProject(input: {
    projectId: string;
  ...
  async pushClientRequest(input: {
    requestId: string;
    organizationId: string;
  }) {
    await recordOutbox(
      "support_request.pushed",
      "SupportRequest",
      input.requestId,
      input,
    );
...
export class LocalClientPortalSyncService implements ClientPortalSyncService {
  async syncProject(projectId: string) {
    await recordOutbox("project.sync_requested", "Project", projectId, {
      projectId,
    });
  }
}
