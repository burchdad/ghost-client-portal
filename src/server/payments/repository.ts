...
export async function getProposalPaymentContextByToken(token: string, db: PrismaClient = getDb()) {
...
export async function getProposalPaymentContextByTokenWithFallback(token: string, db: PrismaClient = getDb()) {
  try {
...
export async function getLatestDepositPaymentForToken(token: string, db: PrismaClient = getDb()) {
  ...
  const deposit = proposal.paymentSchedule.find((item) => item.paymentType === "DEPOSIT");
  const payment = deposit
    ? proposal.payments.find((item) => item.paymentScheduleItemId === deposit.id)
    : undefined;
