...
import { markPaymentFailed, markPaymentRefunded } from "@/server/payments/transitions";
...
import { clientPaymentConfirmationEmail, internalPaymentNotificationEmail, paymentFailureEmail } from "@/server/email/templates";
...
export async function processStripeEvent(event: Stripe.Event) {
  ...
  const stripeEvent = existing ?? await db.stripeEvent.create({
    data: {
  ...
  try {
    ...
    await db.stripeEvent.update({
      ...
      data: { processedAt: new Date(), processingStatus: "PAID", processingNote: "Processed safely." },
    });

    return { ok: true };
  } catch (error) {
    const message = error instanceof Error ? error.message.slice(0, 300) : "Unknown webhook processing error.";
    await db.stripeEvent.update({
      where: { id: stripeEvent.id },
      data: {
...
async function handleCheckoutSessionCompleted(session: Stripe.Checkout.Session) {
  ...
  await getDb().$transaction(async (tx) => {
    const payment = await tx.payment.findUniqueOrThrow({
      ...
      include: { paymentScheduleItem: true, proposal: true, organization: true },
    });
    ...
    assertStripePaymentMatches(payment.amountCents, payment.currency, amountTotal, currency);
    ...
    await tx.outboxEvent.create({
      data: {
        eventType: session.payment_status === "paid" ? "payment.completed" : "payment.processing",
...
async function handleCheckoutSessionExpired(session: Stripe.Checkout.Session) {
  ...
  await getDb().$transaction(async (tx) => {
    const payment = await tx.payment.findUnique({ where: { id: metadata.internalPaymentId } });
    ...
    await tx.paymentScheduleItem.update({
      where: { id: metadata.paymentScheduleItemId },
      data: { status: "PENDING" },
    });
  });
}
...
async function handlePaymentIntentSucceeded(intent: Stripe.PaymentIntent) {
  ...
  await getDb().$transaction(async (tx) => {
    const payment = await tx.payment.findUniqueOrThrow({
      where: { id: metadata.internalPaymentId },
      include: { paymentScheduleItem: true, proposal: true, organization: { include: { contacts: true } } },
    });
    ...
    assertStripePaymentMatches(payment.amountCents, payment.currency, amountReceived, currency);
    ...
    const depositPaidProposal = payment.proposal.status === "PAYMENT_PENDING"
    ...
    try {
      const project = await activateProjectForDeposit(tx, { proposal: depositPaidProposal ?? payment.proposal, payment: paidPayment });
      await tx.payment.update({ where: { id: payment.id }, data: { projectId: project.id } });
      await transitionProposalStatus(tx, {
        proposalId: payment.proposal.id,
        organizationId: payment.proposal.organizationId,
        from: "DEPOSIT_PAID",
      ...
      await tx.payment.update({
        ...
        data: {
          ...
          recoveryReason: error instanceof Error ? error.message.slice(0, 240) : "Project activation failed.",
        },
      });
    ...
    await Promise.all([
      ...
      tx.auditLog.create({
        data: {
          ...
          entityType: "Payment",
          entityId: payment.id,
          metadata: { stripePaymentIntentId: redactStripeId(intent.id), projectActivationStatus },
        },
      }),
    ...
    const primaryEmail = payment.organization.contacts.find((contact) => contact.isPrimary)?.email;
    const provider = getEmailProvider();
    const remaining = Math.max(payment.proposal.totalCents - payment.amountCents, 0);
    ...
    await provider.send({ to: primaryEmail, idempotencyKey: `email:client-payment:${payment.id}`, ...clientEmail });
    await provider.send({ to: process.env.EMAIL_FROM, idempotencyKey: `email:internal-payment:${payment.id}`, ...internalEmail });
  });
}
...
async function handlePaymentIntentFailed(intent: Stripe.PaymentIntent) {
  ...
  await getDb().$transaction(async (tx) => {
    ...
    await getEmailProvider().send({ to: null, idempotencyKey: `email:payment-failed:${payment.id}`, ...email });
  });
}
...
async function handleChargeRefunded(charge: Stripe.Charge) {
  ...
  await getDb().$transaction(async (tx) => {
    const payment = await tx.payment.findFirst({ where: { stripePaymentIntentId: paymentIntentId } });
    if (!payment) {
      return;
    }

    const refundedAmountCents = charge.amount_refunded ?? 0;
    const status = paymentStatusFromRefund(payment.amountCents, refundedAmountCents);
    if (status === "PAID") {
      return;
    }
    ...
    await tx.auditLog.create({
      data: {
        ...
        entityType: "Payment",
        entityId: payment.id,
        metadata: { refundedAmountCents, stripeChargeId: redactStripeId(charge.id), internalReviewRequired: true },
      },
    });
    await tx.outboxEvent.create({
      data: {
...
function safeEventPayload(event: Stripe.Event) {
  const object = event.data.object as { id?: string; object?: string; metadata?: Record<string, string> };
