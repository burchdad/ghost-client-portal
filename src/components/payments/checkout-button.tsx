...
export function CheckoutButton({
  ...
  const [state, action] = useActionState(createCheckoutSessionAction, { error: null });

  return (
    <form action={action} className="space-y-3">
      <input type="hidden" name="token" value={token} />
      {state.error ? <p className="rounded-md border border-red-300/40 bg-red-950/30 p-3 text-sm text-red-100">{state.error}</p> : null}
